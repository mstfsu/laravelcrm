 ...
 * @license MIT
 */
class Ipv4Subnet implements Countable
{
  /**
  ...
  public function __construct($n=null,$s=null) {
    if ($n instanceof Ipv4Address) $n = $n->toString();
    if ($s instanceof Ipv4Address) $s = $s->toString();
  ...
  static function ContainsAddress($sn,$ip) {
    if (is_string($sn)) $sn = Ipv4Subnet::fromString($sn);
    if (is_string($ip)) $ip = Ipv4Address::fromString($ip);
    if (!$sn instanceof Ipv4Subnet) throw new Exception(self::ERROR_SUBNET_FORMAT);
    if (!$ip instanceof Ipv4Address) throw new Exception(Ipv4Address::ERROR_ADDR_FORMAT);
  ...
  public function setNetwork($data) {
    $this->nw = Ipv4Address::fromString($data)->toLong();
    return $this;
  }
  ...
  public function setNetmask($data) {
    $data = Ipv4Address::fromString($data);
  ...
  public function getNetwork() {
    $nw_bin = Ipv4Address::fromLong($this->nw)->toBinary();
    $nw_bin = (str_pad(substr($nw_bin,0,$this->getNetmaskCidr()),32,0));
    return Ipv4Address::fromBinary($nw_bin)->toString();
  }
  ...
  public function getFirstHostAddr() {
    $bin_net = Ipv4Address::fromString($this->getNetwork())->toBinary();
    $bin_first = (str_pad(substr($bin_net,0,31),32,1));
    return Ipv4Address::fromBinary($bin_first)->toString();
  }
  ...
  public function getLastHostAddr() {
    $bin_bcast = Ipv4Address::fromString($this->getBroadcastAddr())->toBinary();
    $bin_last = (str_pad(substr($bin_bcast,0,31),32,0));
    return Ipv4Address::fromBinary($bin_last)->toString();
  }
  ...
  public function getBroadcastAddr() {
    $bin_host = Ipv4Address::fromLong($this->nw)->toBinary();
    $bin_bcast = str_pad(substr($bin_host,0,$this->getNetmaskCidr()),32,1);
    return Ipv4Address::fromBinary($bin_bcast)->toString();
  }
  ...
  public function getIterator() {
   ...
   //if (!class_exists('Ipv4_SubnetIterator'))
    ...
    return new Ipv4SubnetIterator($this);
  }
