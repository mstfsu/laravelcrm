...
function sendForm() {
    ...
    $.ajax({
        url: "/classification_group_add_agent",
        type: "POST",
        data: {
        ...
        success: function (response) {
            if (response == 'false') {
                toastr['error']('Error! Agent is already added', {
                    closeButton: true,
                    tapToDismiss: false,
                });
            } else {
                toastr['success']('Success! Agent is added', {
                    closeButton: true,
                    tapToDismiss: false,
                });

                var parameter = "'" + response['agent_id'] + "'" + "," + "'" + $('#group_id').val() + "'";

                var id = response['agent_id'] + response['group_id'];
                var adminDiv = '<div id="adminDiv' + id + '" class="col-12">' + response['agent_name'] + ' (<span class="text-primary" id="level'+id+'">' + response['level_name'] + '</span>) ' +
                    '<a href="javascript:void(0)" title="Delete Admin" onclick="deleteAgent(' + parameter + ')"><span' + 'data-feather="trash">' + '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>'
                    + '</span></a>'+
...
function deleteAgent(agent_id, group_id) {
    $.ajax({
        url: "/delete_agent_from_group",
        data: {agent_id: agent_id, group_id: group_id},
        success: function (response) {
            toastr['success']('Success! Agent is removed', {
                closeButton: true,
                tapToDismiss: false,
            });
            $('#adminDiv' + agent_id + group_id).remove();
        }
    });
}
...
function editAdminLevel() {
    ...
    $.ajax({
        url: "/change_agent_level",
        data: {agent_id: idOfAgent, group_id: idOfGroup, level: level},
        success: function (response) {
            ...
            console.log(level);
            console.log(idOfAgent+idOfGroup);
            $("#level" + idOfAgent + idOfGroup).text(level);
        }
    });
}
